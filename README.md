...
| `starship` | both | `starship.toml` | One prompt config for zsh and PowerShell — path, git status, `❯` on its own line |
...
- **Both platforms run the literal same prompt.** `starship/.config/starship.toml` is one file, read directly by both `.zshrc` and the PowerShell profile via `STARSHIP_CONFIG` — not two matching configs, the same one. It replaced Powerlevel10k and oh-my-posh, both of which needed a separate file kept in sync by hand.
- **Git Bash is deliberately left alone.** It keeps whatever prompt Git for Windows ships by default, and this repo writes no `.bashrc` or `.bash_profile` for it. Starship was wired in there at one point, but Readline's own multi-line-prompt redraw bug (below) made it more trouble than it was worth, so that was pulled back out.
- **The prompt sticks to Starship's own defaults, including the Nerd Font branch glyph** — both setup scripts install FiraCode Nerd Font, so it's always there by the time this config is read. The one deliberate exception: only `directory`, `git_branch`, `git_status` and `character` are shown; everything else Starship displays by default (username, language versions, battery, ...) is left out, a minimal prompt on purpose. Within those four modules, every colour, symbol and format string is whatever `starship print-config --default` says it is — verified directly against the binary, not assumed. `$character` sits on its own line rather than trailing the rest inline, which keeps line 1 short enough to be less likely to wrap when the terminal narrows — wrapped content is exactly what the resize-redraw bug below needs to misfire.
- **Resizing the terminal can still misplace the cursor.** On Windows, PSReadLine tracks where your input begins as a buffer coordinate; resize a window while the prompt is wrapped and its idea of that position stops matching what's on screen. This is [PSReadLine #3637](https://github.com/PowerShell/PSReadLine/issues/3637), still open, with no setting that avoids it — **F5** is bound in the profile to `InvokePrompt()`, which redraws the prompt in place without wiping the scrollback the way Ctrl+L does. macOS has the same class of bug in zsh itself, documented in [Powerlevel10k's own FAQ](https://github.com/romkatv/powerlevel10k/blob/master/README.md) as "Horrific mess when resizing terminal window" — zsh redraws at a stale line offset after a resize reflows the prompt. Git Bash's line editor, GNU Readline, has its [own documented version](https://github.com/junegunn/fzf/issues/490) of the same thing — a multi-line prompt redraws with the first line overwritten by the last — which is exactly why Git Bash is left unconfigured above rather than given the same Starship prompt. Neither PSReadLine's nor zsh's version is a defect in this config or in Starship; both live below the prompt layer, in the shell's own line editor and the terminal's reflow racing each other. The bug specifically needs line 1 (`path branch [status]`) to wrap onto a second physical row on its own — a prompt that's already two logical lines is fine as long as each one fits in a single row. A short two-line prompt with no right-hand side pushes out how narrow the terminal has to get before that wrap happens, but for an arbitrarily narrow window it can't be prevented entirely — no static prompt length guarantees zero wrapping for every branch name and folder name, which is also why `directory.truncation_length` is left at Starship's own default (3) rather than cut further for this alone. VS Code's own integrated terminal has a separate, unrelated resize bug: its renderer (xterm.js) can duplicate prompt lines when reflowing its scrollback buffer, most reliably when shrinking the window and then growing it back. That is a bug in xterm.js's buffer reflow, not in this config, this repo's shells, or VS Code's `settings.json` (which this repo does not manage); `clear`/`Ctrl+L` clears the duplicate without losing scrollback content.
